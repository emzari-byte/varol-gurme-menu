...
class ModelCatalogProduct extends Model {
		...
		if ($field == 'name') {
			...
			$this->db->query("UPDATE " . DB_PREFIX . "product
				SET location = '" . $this->db->escape($value) . "',
					date_modified = NOW()
				WHERE product_id = '" . (int)$product_id . "'");
		} elseif (in_array($field, array('upc', 'ean', 'jan', 'isbn'))) {
			$this->db->query("UPDATE " . DB_PREFIX . "product
	...
	public function quickUpdateProductDescriptions($product_id, $descriptions) {
		foreach ($descriptions as $language_id => $value) {
			$language_id = (int)$language_id;
			$current = $this->db->query("SELECT * FROM " . DB_PREFIX . "product_description
				WHERE product_id = '" . (int)$product_id . "'
					AND language_id = '" . $language_id . "'
				LIMIT 1");

			$name = isset($value['name']) ? $value['name'] : ($current->num_rows ? $current->row['name'] : '');
			$description = isset($value['description']) ? $value['description'] : ($current->num_rows ? $current->row['description'] : '');
			$meta_title = isset($value['meta_title']) ? $value['meta_title'] : ($current->num_rows ? $current->row['meta_title'] : $name);
			$meta_description = isset($value['meta_description']) ? $value['meta_description'] : ($current->num_rows ? $current->row['meta_description'] : '');
			$meta_keyword = isset($value['meta_keyword']) ? $value['meta_keyword'] : ($current->num_rows ? $current->row['meta_keyword'] : '');
			$tag = isset($value['tag']) ? $value['tag'] : ($current->num_rows ? $current->row['tag'] : '');

			if ($meta_title === '') {
				$meta_title = $name;
			}

			$this->db->query("REPLACE INTO " . DB_PREFIX . "product_description SET
	...
	public function quickUpdateProductCategories($product_id, $categories) {
		$this->db->query("DELETE FROM " . DB_PREFIX . "product_to_category WHERE product_id = '" . (int)$product_id . "'");
	...
	public function getProducts($data = array()) {
		...
		if (!isset($data['sort']) || $data['sort'] == 'category_name') {
			$sql .= ", pd.name ASC";
		}
