...
class ControllerCatalogProduct extends Controller {
	...
	public function quickUpdate() {
		...
		if (!$this->user->hasPermission('modify', 'catalog/product')) {
			...
			if (!$product_id || !in_array($field, $allowed)) {
			...
			} elseif ($field == 'model' && utf8_strlen(trim($value)) < 1) {
				$json['error'] = 'Bu alan boş bırakılamaz.';
			} else {
				if ($field == 'name' || $field == 'description') {
					foreach ($product_description as $language_id => $description) {
						if ($field == 'name' && empty($description['name'])) {
					...
					$this->model_catalog_product->quickUpdateProductCategories($product_id, $product_category);
				} else {
					$this->model_catalog_product->quickUpdateProductField($product_id, $field, $value);
				}
				...
				if ($field == 'price' && $product_info) {
					$json['value'] = $this->currency->format($product_info['price'], $this->config->get('config_currency'));
				} elseif ($field == 'status' && $product_info) {
					$json['value'] = $product_info['status'] ? $this->language->get('text_enabled') : $this->language->get('text_disabled');
					$json['status'] = (int)$product_info['status'];
				} elseif ($field == 'image' && $product_info) {
					if (is_file(DIR_IMAGE . $product_info['image'])) {
						$json['thumb'] = $this->model_tool_image->resize($product_info['image'], 40, 40);
					} else {
						$json['thumb'] = $this->model_tool_image->resize('no_image.png', 40, 40);
					}
				} elseif ($field == 'category') {
					...
					foreach ($this->model_catalog_product->getProductCategories($product_id) as $category_id) {
						...
						if ($category_info) {
							$category_names[] = html_entity_decode($category_info['name'], ENT_QUOTES, 'UTF-8');
						}
					}

					$json['value'] = $category_names ? implode(' > ', $category_names) : '-';
				} else {
					$json['value'] = $field == 'name' && $product_info ? $product_info['name'] : $value;
				}
			}
		}
	...
	public function quickDescription() {
		...
		if (!$this->user->hasPermission('access', 'catalog/product')) {
			...
			if (!$product_info) {
				...
				usort($json['languages'], function($a, $b) {
					if ($a['code'] == 'tr-tr') {
						return -1;
					}
				...
				$json['descriptions'] = $this->model_catalog_product->getProductDescriptions($product_id);
			}
		}
	...
	public function quickCategory() {
		...
		if (!$this->user->hasPermission('access', 'catalog/product')) {
			...
			$json['selected'] = $this->model_catalog_product->getProductCategories($product_id);
	...
	protected function getList() {
		...
		$akinsoft_sync_map = $this->getAkinsoftProductSyncMap($results);
		...
		foreach ($results as $result) {
			...
			$data['products'][] = array(
				...
				'akinsoft_sync_price' => isset($akinsoft_sync_map[$result['product_id']]) ? $akinsoft_sync_map[$result['product_id']]['akinsoft_price'] : '',
				'akinsoft_sync_date' => isset($akinsoft_sync_map[$result['product_id']]) ? $akinsoft_sync_map[$result['product_id']]['date_synced'] : '',
		...
		$data['sort_category'] = $this->url->link('catalog/product', 'user_token=' . $this->session->data['user_token'] . '&sort=category_name' . $url, true);
	...
	private function getAkinsoftProductSyncMap($products) {
		...
		$table_query = $this->db->query("SHOW TABLES LIKE '" . $this->db->escape(DB_PREFIX . "restaurant_akinsoft_product_sync") . "'");

		if (!$table_query->num_rows) {
			return array();
		}

		$query = $this->db->query("SELECT product_id, model, status, akinsoft_price, old_price, date_synced
			FROM `" . DB_PREFIX . "restaurant_akinsoft_product_sync`
